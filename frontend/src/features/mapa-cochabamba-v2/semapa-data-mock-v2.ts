// ============================================================
// SEMAPA COCHABAMBA â DATOS MAESTROS Y LÃGICA DE NEGOCIO
// Fuente: Tarifario.csv + PrÃ¡ctica 5 Arquitectura Cassandra
// ============================================================
...
} from './semapa-map-types-v2';
import { esConsumoExcesivo, calcularAntiguedadAnios } from './map-utils-v2';

// âââ Tarifario SEMAPA (Tarifario.csv) âââââââââââââââââââââââââââââââââââââââââ
// cargo_fijo cubre los primeros 12mÂ³ base
// tramos: 13-25 | 26-50 | >50 mÂ³
export interface TarifaDetalle {
  readonly cargo_fijo:       number;   // Bs
  readonly tramo1_limite:    number;   // mÂ³ mÃ¡x tramo 1 (sobre los 12 base)
  readonly tramo1_precio:    number;   // Bs/mÂ³
  readonly tramo2_limite:    number;   // mÂ³ mÃ¡x tramo 2 acumulado
  readonly tramo2_precio:    number;   // Bs/mÂ³
  readonly tramo3_precio:    number;   // Bs/mÂ³ (>tramo2_limite)
}
...
// âââ Algoritmo de facturaciÃ³n (Tarifario.csv rules) ââââââââââââââââââââââââââ
/**
 * Calcula la factura mensual aplicando el tarifario de 9 categorÃ­as.
 * - cargo_fijo: cubre los primeros 12mÂ³
 * - Si consumo > 12mÂ³: aplica precio por tramos sobre el excedente
 * - Si consumo > 45mÂ³: marca como excesivo (ONU)
 */
export function calcularFactura(categoria: TarifaCategoria, consumoM3: number): Factura {
  ...
  const excedente    = Math.max(0, consumoM3 - 12); // primeros 12mÂ³ cubiertos por cargo fijo
...
// âââ Datos de Distritos (15 distritos Cochabamba â 6 Comunas) ââââââââââââââââ
// Orden numÃ©rico. SubalcaldÃ­a = nombre correcto de comuna.
export const DISTRITOS: DistritoMetrics[] = [
  ...
  { id:4,  name:'D-4 HipÃ³dromo',      subalcaldia:'Molle',            consumoM3:285, presionPSI:11.8, poblacion:44800,  medidoresActivos:6600,  medidoresTotal:7100,  cobertura:92.9, calidadICA:76, temperatura:19.1, status:'mantenimiento' },
...
// âââ Modelos de medidores â stats tÃ©cnicos ââââââââââââââââââââââââââââââââââââ
...
// âââ Radiobases LoRaWAN (32 GAMC) ââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Inmuebles / Contratos mock âââââââââââââââââââââââââââââââââââââââââââââââ
// Simula SELECT â¦ WHERE id_servicio=? (Cassandra primary key lookup)
export const INMUEBLES: Inmueble[] = [
  { contrato:'CBB-00123456', ci:'7890123', nombre:'GarcÃ­a Vargas, MarÃ­a Elena',   direccion:'Av. Ayacucho 1234',        zona:'Zona 1', distritoId:10, categoria:'R3-Residencial',      medidorSerie:'ITC100-123456',   medidorModelo:'ITC 100',  instalacion:'2021-03-15', estadoServicio:'al-dia',    consumoActualM3:22,  consumoAnteriorM3:19,  deudaTotal:0     },
  ...
  { contrato:'CBB-00672341', ci:'8901234', nombre:'Quispe Flores, Ana MarÃ­a',     direccion:'Pasaje Sucre 23',          zona:'Zona 4', distritoId:5,  categoria:'R3-Residencial',      medidorSerie:'LAIN-672341',     medidorModelo:'LAIN IoT', instalacion:'2022-11-30', estadoServicio:'moroso',    consumoActualM3:18,  consumoAnteriorM3:17,  deudaTotal:1960  },
  ...
  { contrato:'CBB-00334455', ci:'2234567', nombre:'Mamani Condori, LucÃ­a',        direccion:'Calle Colombia 331',       zona:'Zona 3', distritoId:1,  categoria:'R1-Preferencial',     medidorSerie:'Siconia-334455',  medidorModelo:'Siconia',  instalacion:'2023-07-01', estadoServicio:'al-dia',    consumoActualM3:10,  consumoAnteriorM3:9,   deudaTotal:0     },
];
...
// âââ Ranking de Morosos âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
export const MOROSOS: Moroso[] = [
  ...
  { contrato:'CBB-00672341', nombre:'Quispe Flores, Ana MarÃ­a',   zona:'Zona 4', distritoId:5,  categoria:'R3-Residencial',      deudaTotal:1960,  mesesDeuda:4,  ultimoPago:'2025-01-10', medidorSerie:'LAIN-672341' },
];

// âââ Cierre Mensual (DÃ­a 10 de cada mes) âââââââââââââââââââââââââââââââââââââ
...
// âââ HistÃ³rico consumo ciudad (12 meses) âââââââââââââââââââââââââââââââââââââ
...
// âââ ProyecciÃ³n demanda 5 aÃ±os (crecimiento 2.6% anual) ââââââââââââââââââââââ
