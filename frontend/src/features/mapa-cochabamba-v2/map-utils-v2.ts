import type { PaqueteLora } from './semapa-map-types-v2';

// âââ CSS class merge ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
export function cn(...classes: (string | undefined | null | false | 0)[]): string {
  return classes.filter(Boolean).join(' ');
}

// ============================================================
// FILTRO DE INGESTA LORAWAN â LIMPIEZA 0.07% DUPLICADOS
// Simula el procesamiento de 21.6M registros/mes de 32 radiobases
// Cassandra query pattern: SELECT â¦ WHERE sensor_id=? AND ts=?
// ============================================================

/**
 * Filtra registros duplicados que llegan cuando dos radiobases LoRaWAN
 * capturan la misma transmisiÃ³n. Duplicados esperados: 0.07% del total.
 *
...
export function filtrarDuplicadosLora(paquetes: PaqueteLora[]): {
  ...
  for (const p of paquetes) {
    // Clave de deduplicaciÃ³n: medidor + ventana de 5 minutos
 ...
 * PatrÃ³n: 21.6M/mes Ã· 30 dÃ­as Ã· 24 horas = ~30,000 paquetes/hora
 */
  ...
  for (let i = 0; i < cantidad; i++) {
    ...
    if (Math.random() < 0.0007) {
      paquetes.push({
        ...
        timestamp:    ts + Math.floor(Math.random() * 120_000), // Â±2 min
        consumo:      parseFloat((Math.random() * 5 + 0.5).toFixed(3)),
        rssi:         -(60 + Math.floor(Math.random() * 40)),
      });
    }
...
// ============================================================
// DETECCIÃN DE ANOMALÃAS â CONSUMO EXCESIVO
// ParÃ¡metro ONU: >45mÂ³/mes es consumo excesivo per cÃ¡pita
// ============================================================

export const UMBRAL_EXCESO_ONU_M3 = 45;

/**
 * Determina si un consumo mensual supera el umbral ONU (45mÂ³).
 */
...
/** Convierte string ISO date a antigÃ¼edad en aÃ±os */
...
/**
 * VirtualizaciÃ³n simple: extrae un slice de datos para tabla paginada
 * sin bloquear el hilo principal (sin usar Worker aquÃ­, pero ready para Web Worker).
 */
