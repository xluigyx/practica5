...
import { COCHABAMBA_ZONE_PATHS, type ZonePath } from '@/src/lib/cochabamba-zone-paths';
import {
  COMUNAS,
  getComunaByDistritoId,
  type MapMetric, MAP_METRIC_LABELS, MAP_METRIC_INVERT,
} from '@/src/lib/semapa-territory';
import type { DistritoMetrics } from '@/src/lib/types';
...
// ─── Pastel district colors (familia cromática por comuna) ────────────────────
const DISTRICT_PASTEL: Record<number, string> = {
  1:  '#BFE8B8', // Tunari  – D1  (verde suave)
  2:  '#A9DFA2', // Tunari  – D2  (verde medio)
  13: '#8FD48A', // Tunari  – D13 (verde más oscuro)
  3:  '#BFD8F2', // Molle   – D3  (azul suave)
  4:  '#9FC5E8', // Molle   – D4  (azul medio)
  10: '#F8E7A3', // Adela Z – D10 (amarillo suave)
  11: '#F5D87A', // Adela Z – D11 (amarillo medio)
  12: '#F2C85E', // Adela Z – D12 (ámbar)
  6:  '#AEE3E0', // Valle H – D6  (celeste suave)
  7:  '#8DD6D2', // Valle H – D7  (turquesa medio)
  14: '#70C7C4', // Valle H – D14 (turquesa más oscuro)
  5:  '#F6C6A6', // Alejo C – D5  (naranja suave)
  8:  '#F0AA7E', // Alejo C – D8  (naranja medio)
  9:  '#D8BDE8', // Itocta  – D9  (lila suave)
  15: '#C7A3DA', // Itocta  – D15 (lila medio)
};
...
// Paleta heatmap pastel: menta → amarillo → ámbar → coral
function lightHeatColor(t: number): string {
  const c = Math.max(0, Math.min(1, t));
  const stops: [number,number,number][] = [
    [187, 247, 208], // #bbf7d0 – menta
    [254, 249, 195], // #fef9c3 – amarillo muy suave
    [253, 230, 138], // #fde68a – ámbar suave
    [252, 165, 165], // #fca5a5 – coral suave
  ];
...
export default function CochabambaMap({
  ...
  const [viewMode, setViewMode] = useState<ViewMode>('commune');
  ...
  // ── Refs for smooth animation (no React re-render per frame) ───────────────
  ...
  // ── RAF animation loop — smooth zoom/pan via direct setAttribute ───────────
  useEffect(() => {
    let raf: number;
    ...
    const tick = (now: number) => {
      ...
      last = now;
      // Exponential ease: ~18% closer per frame at 60fps → ~300ms feel
  ...
  // ── Compute district bboxes & centroids via getBBox (post-mount) ───────────
  useEffect(() => {
  ...
  // ── Memoised heatmap fills (recomputed when metric changes) ────────────────
  ...
  const handleZoneEnter = useCallback((e: React.MouseEvent, zone: ZonePath) => {
    ...
    const metricVal = distrito ? formatMetricValue(getMetricValue(distrito, metric), metric) : '—';
    ...
    <div className={cn('relative flex flex-col h-full', className)}>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-outline-variant bg-surface-container-low/40 flex-shrink-0 overflow-hidden">
        ...
        {selectedCommune && selectedDistrict ? (
          <>
            <span className="text-on-surface-variant/40 text-sm flex-shrink-0">›</span>
            <span
            ...
            <span className="text-[11px] font-semibold text-on-surface truncate">
              D{selectedDistritoId} · {districtShortName}
            </span>
          </>
        ) : (
          <span className="text-[10px] text-on-surface-variant">54 zonas · 15 distritos · 6 comunas</span>
        )}
      ...
      >
        ...
        >
          <defs>
            {/* Subtle cartographic grid — light theme */}
          ...
          {COCHABAMBA_ZONE_PATHS.map((zone) => {
            ...
            // Stroke: 3 levels — zone / district hover / district selected
            const stroke = isSel
              ? communeColorDark
              : isHov
                ? `${communeColorDark}cc`
          ...
          {/* ── Selection accent ring (rendered above all zones) ───────────── */}
      ...
      <div className="px-4 pt-2.5 pb-3 border-t border-outline-variant bg-surface-container-low/20 flex-shrink-0 space-y-2">
        {/* Heatmap bar — only in heatmap mode */}
        {viewMode === 'heatmap' && (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex justify-between mt-0.5">
                {(MAP_METRIC_INVERT[metric]
                  ? ['Bajo', 'Normal', 'Alto', '⚠ Crít.']
                  : ['⚠ Crít.', 'Alto', 'Normal', 'Bajo']
                ).map((l, i) => (
